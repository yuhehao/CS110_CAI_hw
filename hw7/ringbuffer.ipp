...
{ // the same to read_mult
    ...
    else
    {
        ...
        while (i < rdsize)
        {
            data = _M_content[(_M_read_pos++) % _M_capacity]; /*write to data and update index and read_pos(just like pop)*/
            i++;
        }
    }
...
{
    if (data.size() < rdsize)//check data vector size
        data.resize(rdsize);
    size_t num = 0;
    if (_M_content == nullptr) // check if the content is valid
        return false;          /*whether input is NULL*/
    ...
    else
    {
        ...
        while (i < rdsize)
        {
            data[i++] = (_M_content[(_M_read_pos++) % _M_capacity]); /*write to data and update index and read_pos(just like pop)*/
        }
    }
    return true; /*if we finish the part return true*/
}
...
{
    size_t i = 0;
    if (data.size() == 0 || wrtsize > data.size())
        return false;   /*whether input is nullptr or wrtsize > data.size*/
...
template <typename _Tp>
class __detail::__const_iterator // const iterator
{
public:
    // constructor
    __const_iterator() : _ptr(nullptr), _buffer(nullptr) {}
    ...
    __const_iterator(_Tp *ptr, const RingBuffer<_Tp> *buffer) : _ptr(ptr) // constructor
    {
        _buffer = buffer;
        _index = _ptr - _buffer->_M_content;
    }

    ~__const_iterator() = default; // deconstructor

    __const_iterator<_Tp> operator+(int n) // Overload+
    {
        return __const_iterator(&_buffer->_M_content[(_index + n) % _buffer->_M_capacity], _buffer); // compute ptrs
    }
    __const_iterator<_Tp> operator-(int n) // Overload-
    {
        return __const_iterator(&_buffer->_M_content[(_index - n) % _buffer->_M_capacity], _buffer); // compute ptr
    }

    __const_iterator<_Tp> &operator=(const __const_iterator &x) // Overload=
    {
        _ptr = x._ptr;
        _index = x._index;
        _buffer = x._buffer;
    ...
    // ++i
    __const_iterator<_Tp> operator++() // Overload ++i
    {
        _index++;
        _ptr = _buffer->_M_content + (_index) % _buffer->_M_capacity; // compute ptr
        return *this;
    ...
    // i++
    __const_iterator<_Tp> operator++(int) // Overload i++
    {
        __const_iterator Copy(_ptr, _buffer);
        _index++;
        _ptr = _buffer->_M_content + (_index) % _buffer->_M_capacity; // compute ptr
        return Copy;
    }
    ...
    bool operator==(const __const_iterator &x) // Overload == compare
    {
        return _ptr == x._ptr;
    }
